...
const C = { saffron:'#F4A020', saffronPale:'#1a1a1a0CC', maroon:'#800020', maroonL:'#B00030', cream:'#FDF6EC', warmWhite:'#1a1a1a9F0', textDark:'#2C1810', textMuted:'#6B4C3B', textFaint:'#A68070', border:'rgba(128,0,32,0.12)' };
...
export default function MyAssetsPage() {
  ...
  if (loading) return (
    <div style={{ minHeight:'100vh', background:'transparent', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16 }}>
      <div style={{ width:44, height:44, border:'3px solid rgba(0,0,0,0.2)', borderTopColor:C.saffron, borderRadius:'50%', animation:'spin 0.8s linear infinite', zIndex:1 }} />
      <p style={{ color:C.textMuted, fontFamily:"'Outfit',sans-serif", zIndex:1 }}>Loading assets…</p>
  ...
  return (
    <div style={{ background:'transparent', minHeight:'100vh', position:'relative', fontFamily:"'Outfit',system-ui,sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@700&family=Outfit:wght@400;500;600;700;800&display=swap');
        @keyframes fadeUp{from{opacity:0;transform:translateY(18px);}to{opacity:1;transform:translateY(0);}}
        .asset-card{transition:all 0.25s ease;}
        .asset-card:hover{transform:translateY(-4px);box-shadow:0 10px 32px rgba(0,0,0,0.3) !important;}
      `}</style>
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:1200, margin:'0 auto', padding:'2rem 1.5rem' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'2rem', animation:'fadeUp 0.5s ease both' }}>
          <div>
            <h1 style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'2.6rem', fontWeight:700, color:'#1a1a1a', letterSpacing:'-0.02em', margin:0 }}>My Assets</h1>
            <p style={{ color:'rgba(149, 13, 13, 0.7)', marginTop:4 }}>{assets.length} listing{assets.length !== 1 ? 's' : ''}</p>
          </div>
          <button onClick={() => navigate('/my-assets/add')} style={{ padding:'0.75rem 1.5rem', background:C.maroon, color:'#ddd', border:'none', borderRadius:10, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:8 }}>
            <Icons.Plus /> Add Asset
          </button>
        </div>

        {assets.length === 0 ? (
          <div style={{ textAlign:'center', padding:'5rem 2rem', background:'rgba(255,255,255,0.1)', backdropFilter:'blur(16px)', borderRadius:16, border:'1.5px solid rgba(255,255,255,0.2)', animation:'fadeUp 0.5s ease 0.05s both' }}>
            <div style={{ width:64, height:64, margin:'0 auto 1rem', color:'rgba(255,255,255,0.5)' }}><Icons.Package /></div>
            <h3 style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'1.5rem', color:'#1a1a1a', marginBottom:'0.5rem' }}>No assets listed yet</h3>
            <p style={{ color:'rgba(100,0,0,0.6)', marginBottom:'1.5rem' }}>List your first item and start earning from things you own.</p>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:'1.5rem' }}>
            {assets.map((a, i) => (
              <div key={a.asset_id} className="asset-card" style={{ 
                ...
                <div onClick={() => navigate(`/assets/${a.asset_id}`)} style={{ height:200, background: a.primary_image ? `url(${a.primary_image}) center/cover` : `linear-gradient(135deg,rgba(244,160,32,0.3),rgba(128,0,32,0.3))`, cursor:'pointer', position:'relative' }}>
                  {/* Maroon badge - WHITE TEXT */}
                  <div style={{ position:'absolute', top:10, right:10, background: a.availability_status==='available' ? C.maroon : '#6B7280', color:'#1a1a1a', padding:'3px 12px', borderRadius:999, fontSize:'0.72rem', fontWeight:700 }}>
                ...
                <div style={{ padding:'1.1rem 1.25rem' }}>
                  ...
                  <div style={{ display:'flex', gap:'0.5rem' }}>
                    <button onClick={() => navigate(`/my-assets/edit/${a.asset_id}`)} style={{ flex:1, padding:'0.5rem', background:'rgba(59,130,246,0.2)', color:'#93C5FD', border:'1px solid rgba(59,130,246,0.3)', borderRadius:8, cursor:'pointer', fontWeight:600, fontSize:'0.82rem', display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}><Icons.Edit /> Edit</button>
                    <button onClick={() => handleToggle(a.asset_id, a.availability_status)} style={{ flex:1, padding:'0.5rem', background: a.availability_status==='available' ? 'rgba(239,68,68,0.2)' : 'rgba(5,150,105,0.2)', color: a.availability_status==='available' ? '#FCA5A5' : '#6EE7B7', border:`1px solid ${a.availability_status==='available' ? 'rgba(239,68,68,0.3)' : 'rgba(5,150,105,0.3)'}`, borderRadius:8, cursor:'pointer', fontWeight:600, fontSize:'0.82rem', display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}>
                      {a.availability_status==='available' ? <Icons.ToggleOff /> : <Icons.ToggleOn />}
                      {a.availability_status==='available' ? 'Deactivate' : 'Activate'}
                    </button>
                    <button onClick={() => handleDelete(a.asset_id)} style={{ flex:1, padding:'0.5rem', background:'rgba(239,68,68,0.2)', color:'#FCA5A5', border:'1px solid rgba(239,68,68,0.3)', borderRadius:8, cursor:'pointer', fontWeight:600, fontSize:'0.82rem', display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}><Icons.Trash /> Delete</button>
                  </div>
                </div>
              </div>
            ))}
