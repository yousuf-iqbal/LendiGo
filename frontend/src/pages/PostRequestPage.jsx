...
export default function PostRequestPage() {
  ...
  const [form, setForm] = useState({ title:'', description:'', categoryId:'', startDate:'', endDate:'', maxBudget:'', city:'Lahore', area:'', lat:null, lng:null });
  ...
  const handleSubmit = async e => {
    ...
    if (!form.area || !form.lat || !form.lng) {
      setLocationError('Please pick your area on the Lahore map');
      setError('Location on map is required');
      return;
    }
     ...
     {/*} <FloatingBackground variant="minimal" />*/}
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:960, margin:'0 auto', padding:'2rem 1.5rem' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:'2rem', alignItems:'start' }}>
          <div style={{ background:'transparent', border:`1px solid ${C.border}`, borderRadius:20, padding:'2rem', animation:'fadeUp 0.5s ease 0.08s both' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom:'1.1rem' }}>
                ...
                <textarea value={form.description} onChange={e=>set('description',e.target.value)} rows={4} placeholder="Tell them why you need it, when, any special requirements…"
                  style={{ width:'100%', padding:'0.85rem 1rem', borderRadius:12, border:`1.5px solid ${C.border}`, background:'transparent', fontFamily:"'Outfit',sans-serif", fontSize:'0.95rem', color:C.textDark, outline:'none', resize:'vertical' }}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem' }}>
                <div style={{ marginBottom:'1.1rem' }}>
                  ...
                  <select value={form.categoryId} onChange={e=>set('categoryId',e.target.value)}
                    style={{ width:'100%', padding:'0.85rem 1rem', borderRadius:12, border:`1.5px solid ${C.border}`, background:'transparent', fontFamily:"'Outfit',sans-serif", fontSize:'0.95rem', color:C.textDark, outline:'none' }}>
              ...
              <LocationPickerMap
                ...
                error={locationError}
                onChange={(loc) => {
                  setForm(f => ({ ...f, area: loc.area, lat: loc.lat, lng: loc.lng, city: loc.city }));
                  setLocationError('');
                }}
              />
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem', animation:'fadeUp 0.5s ease 0.12s both' }}>
            <div style={{ background:'transparent', border:`1px solid ${C.border}`, borderRadius:16, padding:'1.5rem' }}>
