...
export default function AddAssetPage() {
  ...
  const [form, setForm] = useState({
    ...
    price_per_day: "", deposit: "", city: "Lahore", area: "", lat: null, lng: null,
  });
 ...
 const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append('location', 'Lahore');
      fd.append('area', form.area);
      fd.append('lat', String(form.lat));
      fd.append('lng', String(form.lng));
      ['name', 'description', 'category', 'price_per_day', 'deposit'].forEach((k) => {
          ...
          <div style={{ background: C.warmWhite, border: `1px solid ${C.border}`, borderRadius: 20, padding: "2rem", boxShadow: "0 4px 24px rgba(128,0,32,0.08)", animation: "fadeUp 0.5s ease 0.08s both" }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <LocationPickerMap
                ...
                onChange={(loc) => setForm(f => ({ ...f, area: loc.area, lat: loc.lat, lng: loc.lng, city: loc.city }))}
              />
