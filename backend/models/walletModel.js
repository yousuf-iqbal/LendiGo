...
const walletModel = {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT 
          ...
          t.CreatedAt,
          b.BookingID,
          uFrom.FullName AS FromUser,
          uTo.FullName AS ToUser
        FROM Transactions t
        ...
        WHERE wFrom.UserID = @userID OR wTo.UserID = @userID
  ...
  payForBooking: async (bookingID, amount) => {
    const pool = await poolPromise;
    const result = await pool.request()
  ...
  // Create wallet for new user (if doesn't exist)
