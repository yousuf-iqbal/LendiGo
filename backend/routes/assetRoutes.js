...
const { validateLocation } = require('../utils/lahoreGeo');
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res.json({
      ...
      area: asset.area,
      lat: asset.lat ? parseFloat(asset.lat) : null,
      lng: asset.lng ? parseFloat(asset.lng) : null,
...
router.post('/', verifyToken, upload, async (req, res) => {
  try {
    const { name, description, category, price_per_day, location, area, lat, lng } = req.body;
    const pool = global.pool;
    ...
    if (!locationCheck.valid) {
      return res.status(400).json({ error: locationCheck.errors[0] });
    }
    ...
    const assetResult = await pool.request()
      ...
      .input('City', sql.NVarChar, locationCheck.city)
      .input('Area', sql.NVarChar, locationCheck.area)
      .input('Latitude', sql.Decimal(9, 6), locationCheck.lat)
      .input('Longitude', sql.Decimal(9, 6), locationCheck.lng)
      .input('OwnerID', sql.Int, req.userID)
      .input('IsActive', sql.Bit, 1)
      .query(`
        INSERT INTO Assets (Title, Description, CategoryID, PricePerDay, City, Area, Latitude, Longitude, OwnerID, IsActive)
        OUTPUT INSERTED.AssetID
        VALUES (@Title, @Description, @CategoryID, @PricePerDay, @City, @Area, @Latitude, @Longitude, @OwnerID, @IsActive)
      `);
...
router.put('/:id', verifyToken, upload, async (req, res) => {
  try {
    ...
    if (area !== undefined || lat !== undefined || lng !== undefined) {
      locationCheck = validateLocation({
        area: area || location,
        lat,
        lng,
        city: location || 'Lahore',
    ...
    await pool.request()
      ...
      .input('City', sql.NVarChar, locationCheck?.city || location?.trim())
      .input('Area', sql.NVarChar, locationCheck?.area || area?.trim())
      .input('Latitude', sql.Decimal(9, 6), locationCheck?.lat ?? null)
      .input('Longitude', sql.Decimal(9, 6), locationCheck?.lng ?? null)
      .query(`
        UPDATE Assets 
