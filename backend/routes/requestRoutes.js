...
const pool = await poolPromise; // ✅ Await poolPromise
    const result = await pool.request()
        .query(`SELECT r.RequestID as id, r.Title as title, r.Description as description, r.MaxBudget as maxBudget, r.StartDate as startDate, r.EndDate as endDate, r.Status as status, r.CreatedAt as createdAt, u.FullName as requesterName, u.UserID as requesterId, c.Name as categoryName FROM Requests r LEFT JOIN Users u ON r.RequesterID = u.UserID LEFT JOIN Categories c ON r.CategoryID = c.CategoryID ORDER BY r.CreatedAt DESC`);
    res.json(result.recordset);
} catch (err) {
...
const pool = await poolPromise; // ✅ Await poolPromise
    const result = await pool.request()
        ...
        .query(`SELECT r.RequestID as id, r.Title as title, r.Description as description, r.MaxBudget as maxBudget, r.StartDate as startDate, r.EndDate as endDate, r.Status as status, r.CreatedAt as createdAt, r.City as city, r.Area as area, u.FullName as requesterName, u.UserID as requesterId, c.Name as categoryName, c.CategoryID as categoryId FROM Requests r LEFT JOIN Users u ON r.RequesterID = u.UserID LEFT JOIN Categories c ON r.CategoryID = c.CategoryID WHERE r.RequestID = @RequestID`);
